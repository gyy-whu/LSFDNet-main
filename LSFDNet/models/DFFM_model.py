...
class DFFM(BaseModel):
    def __init__(self, opt):
        ...
        load_path = self.opt['path'].get('pretrain_network_DDFM', None)
        if load_path is not None:
            param_key = self.opt['path'].get('param_key_g', None)
            self.load_network(self.netDF, load_path, self.opt['path'].get('strict_load_g', True), param_key)
        # else:
        #     init_type='orthogonal'
        #     init_weights(self.netDF, init_type)
    ...
    def test(self):
        self.netDF.eval()
        with torch.no_grad():
            self.pred_img, fd = self.netDF(self.feats)
    ...
    def save(self, epoch, current_iter):
        self.save_network([self.netDF], 'net_g', current_iter, param_key=['params'])
