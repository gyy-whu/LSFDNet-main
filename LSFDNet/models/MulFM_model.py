...
class MulFM(BaseModel):
    def __init__(self, opt):
        super(MulFM, self).__init__(opt)
        ...
        # logger.info(f"Pretrained model is successfully loaded from {opt['path']['pretrain_network_C2FM']}")

        self.netDF =  build_network(opt['network_fusion_head'])
        self.netDF = self.model_to_device(self.netDF)
        self.print_network(self.netDF)

        if isinstance(self.netft, (DataParallel, DistributedDataParallel)):
            self.netft = self.netft.module
        else:
            self.netft = self.netft

        if isinstance(self.netDF, (DataParallel, DistributedDataParallel)):
            self.netDF = self.netDF.module
        else:
            self.netDF = self.netDF
    ...
    def _initialize_weights(self):  
        ...
        self.netft.apply(weights_init)
        self.netDF.apply(weights_init)
    ...
    def setup_optimizers(self):
        ...
        optim_ft_params = list(self.netft.parameters())
        optim_DF_params = list(self.netDF.parameters())
        optim_params_g_ft = [{  # add normal params first
            'params': optim_ft_params,
            'lr': train_opt['optimizer']['lr']
        }]        
        optim_params_g_DF = [{  # add normal params first
            'params': optim_DF_params,
            'lr': train_opt['optimizer']['lr']
        }]
        optim_type = train_opt['optimizer'].pop('type')
        lr = train_opt['optimizer']['lr']
        self.optimizer_g_DF = self.get_optimizer(optim_type, optim_params_g_DF, lr)
        self.optimizers.append(self.optimizer_g_DF)
        self.optimizer_g_ft = self.get_optimizer(optim_type, optim_params_g_ft, lr)
        self.optimizers.append(self.optimizer_g_ft)
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.optimizer_g_DF.zero_grad()
        self.optimizer_g_ft.zero_grad()
        loss_dict = OrderedDict() 
        self.feats_SW = self.netft(self.data['SW'])
        self.feats_LW = self.netft(self.data['LW'])
        self.pred_img = self.netDF(self.feats_SW, self.feats_LW)
        ...
        loss_fs.backward()
        self.optimizer_g_DF.step()
        self.optimizer_g_ft.step()
    ...
    def save(self, epoch, current_iter):
        self.save_network([self.netft], 'net_ft_g', current_iter, param_key=['params'])
        self.save_network([self.netDF], 'net_DF_g', current_iter, param_key=['params'])
