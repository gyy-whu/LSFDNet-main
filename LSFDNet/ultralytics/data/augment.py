...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic3(self, labels):
        ...
        for i in range(3):
            ...
            vis_img = labels_patch["vis_img"]
            h, w = labels_patch.pop("resized_shape")

            # Place img in img3
            if i == 0:  # center
                img3 = np.full((s * 3, s * 3, img.shape[2]), 114, dtype=np.uint8)  # base image with 3 tiles
                vis_img3 = np.full((s * 3, s * 3, img.shape[2]), 114, dtype=np.uint8)  # base image with 3 tiles
                h0, w0 = h, w
            ...
            vis_img3[y1:y2, x1:x2] = vis_img[y1 - padh :, x1 - padw :]
        ...
        final_labels["vis_img"] = vis_img3[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        return final_labels
    ...
    def _mosaic4(self, labels):
        ...
        for i in range(4):
            ...
            if i == 0:  # top left
                ...
                vis_img4 = np.full((s * 2, s * 2, vis_img.shape[2]), 114, dtype=np.uint8)
            ...
            vis_img4[y1a:y2a, x1a:x2a] = vis_img[y1b:y2b, x1b:x2b]
            padw = x1a - x1b
            padh = y1a - y1b
    ...
    def _mosaic9(self, labels):
        ...
        for i in range(9):
            ...
            if i == 0:  # center
                ...
                vis_img9 = np.full((s * 3, s * 3, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                h0, w0 = h, w
                c = s, s, s + w, s + h  # xmin, ymin, xmax, ymax (base) coordinates
            elif i == 1:  # top
                c = s, s - h, s + w, s
            ...
            vis_img9[y1:y2, x1:x2] = vis_img[y1 - padh :, x1 - padw :]  # img9[ymin:ymax, xmin:xmax]
            hp, wp = h, w  # height, width previous for next iteration
        ...
        final_labels["vis_img"] = vis_img9[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        return final_labels
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        labels["vis_img"] = (labels["vis_img"] * r + labels2["vis_img"] * (1 - r)).astype(np.uint8)
...
class RandomPerspective:
    ...
    def affine_transform(self, img, vis_img, border):
        """
        ...
        if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
            if self.perspective:
                ...
                vis_img = cv2.warpPerspective(vis_img, M, dsize=self.size, borderValue=(114, 114, 114)) 
            else:  # affine
                img = cv2.warpAffine(img, M[:2], dsize=self.size, borderValue=(114, 114, 114))
                vis_img = cv2.warpAffine(vis_img, M[:2], dsize=self.size, borderValue=(114, 114, 114))
        return img, vis_img,  M, s
    ...
    def __call__(self, labels):
        ...
        img = labels["img"]
        vis_img = labels["vis_img"]
        cls = labels["cls"]
        instances = labels.pop("instances")
        # Make sure the coord formats are right
        instances.convert_bbox(format="xyxy")
        instances.denormalize(*img.shape[:2][::-1])
        #instances.denormalize(*vis_img.shape[:2][::-1])

        border = labels.pop("mosaic_border", self.border)
        self.size = img.shape[1] + border[1] * 2, img.shape[0] + border[0] * 2  # w, h
        # M is affine matrix
        # Scale for func:`box_candidates`
        img, vis_img, M, scale = self.affine_transform(img, vis_img, border)
        #vis_img, M, scale = self.affine_transform(vis_img, border)
...
class RandomHSV:
    ...
    def __call__(self, labels):
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            vis_hue, vis_sat, vis_val = cv2.split(cv2.cvtColor(vis_img, cv2.COLOR_BGR2HSV))
            ...
            vis_hsv = cv2.merge((cv2.LUT(vis_hue, lut_hue), cv2.LUT(vis_sat, lut_sat), cv2.LUT(vis_val, lut_val)))
            cv2.cvtColor(im_hsv, cv2.COLOR_HSV2BGR, dst=img)  # no return needed
            cv2.cvtColor(vis_hsv, cv2.COLOR_HSV2BGR, dst=vis_img)  # no return needed
        return labels
...
class RandomFlip:
    ...
    def __call__(self, labels):
        ...
        labels["vis_img"] = np.ascontiguousarray(vis_img)
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        vis_img = labels.get("vis_img") if image is None else image
        ...
        if shape[::-1] != new_unpad:  # resize
            ...
            vis_img = cv2.resize(vis_img, new_unpad, interpolation=cv2.INTER_LINEAR)
        ...
        vis_img = cv2.copyMakeBorder(
            vis_img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)
        )  # add border
...
class CopyPaste:
    ...
    def __call__(self, labels):
        ...
        if self.p and len(instances.segments):
            ...
            vis_new =np.zeros(vis_img.shape, np.uint8)
            ...
            vis_result = cv2.flip(vis_img, 1)  # augment segments (flip left-right)
            i = cv2.flip(im_new, 1).astype(bool)
            vis_i = cv2.flip(vis_new, 1).astype(bool)
            im[i] = result[i]
            vis_img[vis_i] = vis_result[vis_i]
    ...
    def __call__(self, labels):
        ...
        if len(cls):
            ...
            if self.transform and random.random() < self.p:
                ...
                vis_new = self.transform(image=vis_img, bboxes=bboxes, class_labels=cls)  # transformed
                if len(new["class_labels"]) > 0:  # skip update if no bbox in new im
                    labels["img"] = new["image"]
                    labels["vis_img"] = vis_new["image"]
...
class Format:
    ...
    def __call__(self, labels):
        ...
        labels["vis_img"] = self._format_img(vis_img)
